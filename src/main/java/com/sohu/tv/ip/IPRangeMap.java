 ...
 *
 * <pre>
 *   SortedMap m = Collections.synchronizedSortedMap(new IPRangeMap(...));
 * </pre>
 *
 * <p>
...
public class IPRangeMap<K, V> extends AbstractMap<K, V> implements NavigableMap<K, V>, Cloneable, java.io.Serializable {
    /**
     ...
     */
    public IPRangeMap(Comparator<? super K> comparator) {
     ...
     */
    public IPRangeMap(Map<? extends K, ? extends V> m) {
        comparator = null;
        putAll(m);
    }
     ...
     *             if the specified map is null
     */
    public IPRangeMap(SortedMap<K, ? extends V> m) {
    ...
    final Entry<K, V> getEntry(Object key) {
        ...
        while (p != null) {
            ipRange = (IPRange)p.key;
            cmp = ipRange.compare(k);
            if (cmp < 0) {
                p = p.left;
            } else if (cmp > 0) {
                if(ipRange.contain(k)){
                    return p;
                }
                p = p.right;
            } else {
    ...
    /**
     * Removes the mapping for this key from this IPRangeMap if present.
     *
     * @param key
    ...
    /**
     * Returns a shallow copy of this <tt>IPRangeMap</tt> instance. (The keys and
     * values themselves are not cloned.)
     *
     * @return a shallow copy of this map
     */
    public Object clone() {
        IPRangeMap<K, V> clone = null;
        try {
            clone = (IPRangeMap<K, V>) super.clone();
    ...
    class Values extends AbstractCollection<V> {
        ...
        public int size() {
            return IPRangeMap.this.size();
        }

        public boolean contains(Object o) {
            return IPRangeMap.this.containsValue(o);
        }
    ...
    static final class KeySet<E> extends AbstractSet<E> implements NavigableSet<E> {
        ...
        public Iterator<E> iterator() {
            if (m instanceof IPRangeMap)
                return ((IPRangeMap<E, Object>) m).keyIterator();
            else
                return (Iterator<E>) (((IPRangeMap.NavigableSubMap) m).keyIterator());
        }

        public Iterator<E> descendingIterator() {
            if (m instanceof IPRangeMap)
                return ((IPRangeMap<E, Object>) m).descendingKeyIterator();
            else
                return (Iterator<E>) (((IPRangeMap.NavigableSubMap) m).descendingKeyIterator());
        }
    ...
    /**
     * Base class for IPRangeMap Iterators
     */
    ...
    /**
     * Compares two keys using the correct comparison method for this IPRangeMap.
     */
     ...
     */
    static <K, V> Map.Entry<K, V> exportEntry(IPRangeMap.Entry<K, V> e) {
     ...
     */
    static <K, V> K keyOrNull(IPRangeMap.Entry<K, V> e) {
    ...
    static abstract class NavigableSubMap<K, V> extends AbstractMap<K, V> implements NavigableMap<K, V>, java.io.Serializable {
        ...
        NavigableSubMap(IPRangeMap<K, V> m, boolean fromStart, K lo, boolean loInclusive, boolean toEnd, K hi, boolean hiInclusive) {
            if (!fromStart && !toEnd) {
        ...
        final IPRangeMap.Entry<K, V> absLowest() {
            IPRangeMap.Entry<K, V> e = (fromStart ? m.getFirstEntry() : (loInclusive ? m.getCeilingEntry(lo) : m.getHigherEntry(lo)));
            return (e == null || tooHigh(e.key)) ? null : e;
        }

        final IPRangeMap.Entry<K, V> absHighest() {
            IPRangeMap.Entry<K, V> e = (toEnd ? m.getLastEntry() : (hiInclusive ? m.getFloorEntry(hi) : m.getLowerEntry(hi)));
            return (e == null || tooLow(e.key)) ? null : e;
        }

        final IPRangeMap.Entry<K, V> absCeiling(K key) {
            if (tooLow(key))
                return absLowest();
            IPRangeMap.Entry<K, V> e = m.getCeilingEntry(key);
            return (e == null || tooHigh(e.key)) ? null : e;
        }

        final IPRangeMap.Entry<K, V> absHigher(K key) {
            if (tooLow(key))
                return absLowest();
            IPRangeMap.Entry<K, V> e = m.getHigherEntry(key);
            return (e == null || tooHigh(e.key)) ? null : e;
        }

        final IPRangeMap.Entry<K, V> absFloor(K key) {
            if (tooHigh(key))
                return absHighest();
            IPRangeMap.Entry<K, V> e = m.getFloorEntry(key);
            return (e == null || tooLow(e.key)) ? null : e;
        }

        final IPRangeMap.Entry<K, V> absLower(K key) {
            if (tooHigh(key))
                return absHighest();
            IPRangeMap.Entry<K, V> e = m.getLowerEntry(key);
            return (e == null || tooLow(e.key)) ? null : e;
        }

        /** Returns the absolute high fence for ascending traversal */
        final IPRangeMap.Entry<K, V> absHighFence() {
            return (toEnd ? null : (hiInclusive ? m.getHigherEntry(hi) : m.getCeilingEntry(hi)));
        }

        /** Return the absolute low fence for descending traversal */
        final IPRangeMap.Entry<K, V> absLowFence() {
        ...
        abstract IPRangeMap.Entry<K, V> subLowest();

        abstract IPRangeMap.Entry<K, V> subHighest();

        abstract IPRangeMap.Entry<K, V> subCeiling(K key);

        abstract IPRangeMap.Entry<K, V> subHigher(K key);

        abstract IPRangeMap.Entry<K, V> subFloor(K key);

        abstract IPRangeMap.Entry<K, V> subLower(K key);
        ...
        public final Map.Entry<K, V> pollFirstEntry() {
            IPRangeMap.Entry<K, V> e = subLowest();
        ...
        public final Map.Entry<K, V> pollLastEntry() {
            IPRangeMap.Entry<K, V> e = subHighest();
            Map.Entry<K, V> result = exportEntry(e);
            if (e != null)
                m.deleteEntry(e);
            return result;
        ...
        public final NavigableSet<K> navigableKeySet() {
            ...
            return (nksv != null) ? nksv : (navigableKeySetView = new IPRangeMap.KeySet(this));
        }
        ...
        abstract class EntrySetView extends AbstractSet<Map.Entry<K, V>> {
            ...
            public boolean isEmpty() {
                IPRangeMap.Entry<K, V> n = absLowest();
            ...
            public boolean contains(Object o) {
                ...
                if (!inRange(key))
                    return false;
                IPRangeMap.Entry node = m.getEntry(key);
            ...
            public boolean remove(Object o) {
                ...
                if (!inRange(key))
                    return false;
                IPRangeMap.Entry<K, V> node = m.getEntry(key);
        ...
        abstract class SubMapIterator<T> implements Iterator<T> {
            IPRangeMap.Entry<K, V> lastReturned;
            IPRangeMap.Entry<K, V> next;
            final Object fenceKey;
            int expectedModCount;

            SubMapIterator(IPRangeMap.Entry<K, V> first, IPRangeMap.Entry<K, V> fence) {
            ...
            final IPRangeMap.Entry<K, V> nextEntry() {
                IPRangeMap.Entry<K, V> e = next;
            ...
            final IPRangeMap.Entry<K, V> prevEntry() {
                IPRangeMap.Entry<K, V> e = next;
                if (e == null || e.key == fenceKey)
                    throw new NoSuchElementException();
                if (m.modCount != expectedModCount)
        ...
        final class SubMapEntryIterator extends SubMapIterator<Map.Entry<K, V>> {
            SubMapEntryIterator(IPRangeMap.Entry<K, V> first, IPRangeMap.Entry<K, V> fence) {
                super(first, fence);
            }
        ...
        final class SubMapKeyIterator extends SubMapIterator<K> {
            SubMapKeyIterator(IPRangeMap.Entry<K, V> first, IPRangeMap.Entry<K, V> fence) {
                super(first, fence);
            }
        ...
        final class DescendingSubMapEntryIterator extends SubMapIterator<Map.Entry<K, V>> {
            DescendingSubMapEntryIterator(IPRangeMap.Entry<K, V> last, IPRangeMap.Entry<K, V> fence) {
                super(last, fence);
            }
        ...
        final class DescendingSubMapKeyIterator extends SubMapIterator<K> {
            DescendingSubMapKeyIterator(IPRangeMap.Entry<K, V> last, IPRangeMap.Entry<K, V> fence) {
                super(last, fence);
            }
    ...
    static final class AscendingSubMap<K, V> extends NavigableSubMap<K, V> {
        ...
        AscendingSubMap(IPRangeMap<K, V> m, boolean fromStart, K lo, boolean loInclusive, boolean toEnd, K hi, boolean hiInclusive) {
        ...
        IPRangeMap.Entry<K, V> subLowest() {
            return absLowest();
        }

        IPRangeMap.Entry<K, V> subHighest() {
            return absHighest();
        }

        IPRangeMap.Entry<K, V> subCeiling(K key) {
            return absCeiling(key);
        }

        IPRangeMap.Entry<K, V> subHigher(K key) {
            return absHigher(key);
        }

        IPRangeMap.Entry<K, V> subFloor(K key) {
            return absFloor(key);
        }

        IPRangeMap.Entry<K, V> subLower(K key) {
            return absLower(key);
        }
    }
    ...
    static final class DescendingSubMap<K, V> extends NavigableSubMap<K, V> {
        ...
        DescendingSubMap(IPRangeMap<K, V> m, boolean fromStart, K lo, boolean loInclusive, boolean toEnd, K hi, boolean hiInclusive) {
            super(m, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
        }
     ...
     * previous releases of IPRangeMap that did not support NavigableMap. It
    ...
    private class SubMap extends AbstractMap<K, V> implements SortedMap<K, V>, java.io.Serializable {
        ...
        private Object readResolve() {
            return new AscendingSubMap(IPRangeMap.this, fromStart, fromKey, true, toEnd, toKey, false);
        }
    ...
    /**
     * Returns the first Entry in the IPRangeMap (according to the IPRangeMap's
     * key-sort function). Returns null if the IPRangeMap is empty.
     */
    ...
    /**
     * Returns the last Entry in the IPRangeMap (according to the IPRangeMap's
     * key-sort function). Returns null if the IPRangeMap is empty.
     */
     ...
     */
    static <K, V> IPRangeMap.Entry<K, V> successor(Entry<K, V> t) {
        if (t == null)
            return null;
    ...
    /**
     * Save the state of the <tt>IPRangeMap</tt> instance to a stream (i.e.,
     * serialize it).
     *
     * @serialData The <i>size</i> of the IPRangeMap (the number of key-value
     *             mappings) is emitted (int), followed by the key (Object) and
     *             value (Object) for each key-value mapping represented by the
     *             IPRangeMap. The key-value mappings are emitted in key-order (as
     *             determined by the IPRangeMap's Comparator, or by the keys'
     *             natural ordering if the IPRangeMap has no Comparator).
     */
    ...
    /**
     * Reconstitute the <tt>IPRangeMap</tt> instance from a stream (i.e.,
     * deserialize it).
     */
     ...
     *
     * It is assumed that the comparator of the IPRangeMap is already set prior to
     * calling this method.
     *
     * @param size
     ...
     * size fields of the IPRangeMap are already set prior to calling this method.
     ...
     */
    private final Entry<K, V> buildFromSorted(int level, int lo, int hi, int redLevel, Iterator it, java.io.ObjectInputStream str,
